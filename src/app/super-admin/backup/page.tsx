...
export default function BackupRestore() {
  ...
  const [activeHistory, setActiveHistory] = useState([
    { id: 1, type: 'Backup', date: '2026-03-01 10:00', size: '2.4 MB', status: 'Success' },
    { id: 2, type: 'Restore', date: '2026-02-15 14:30', size: '1.8 MB', status: 'Success' },
  ]);
  ...
  const handleRestore = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!confirm("Restorasi data akan menimpa pengaturan sistem saat ini. Lanjutkan?")) return;
    ...
    try {
      ...
      toast.error("Restorasi Gagal", { description: error.response?.data?.message || "Format file tidak sesuai." });
    } finally {
      setLoading(false);
      // Clear file input
      e.target.value = '';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm relative overflow-hidden group">
            ...
            <div className="relative z-10">
                ...
                <h3 className="text-2xl font-black text-[#001a33] mb-4">Export Database</h3>
                <p className="text-sm text-slate-500 leading-relaxed mb-8">
                    Unduh seluruh data (Universitas, User, Config, dll) dalam format JSON. File ini dapat digunakan untuk merestorasi sistem jika terjadi kendala.
                </p>
                <button 
                  onClick={handleBackup}
                  disabled={loading}
                  className="px-8 py-4 bg-[#094E8B] text-white font-black uppercase text-xs tracking-widest rounded-2xl flex items-center gap-3 hover:bg-[#073e6f] transition-all shadow-xl shadow-blue-900/20 active:scale-95 disabled:opacity-50"
                >
                    {loading ? <CircleNotch className="animate-spin" size={20} /> : <><Database weight="bold" /> Jalankan Backup</>}
                </button>
            </div>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm relative overflow-hidden group">
            ...
            <div className="relative z-10">
                ...
                <h3 className="text-2xl font-black text-[#001a33] mb-4">Restore Data</h3>
                <p className="text-sm text-slate-500 leading-relaxed mb-8">
                    Unggah file backup KonverPro (.json) untuk memulihkan keadaan sistem. Peringatan: Proses ini tidak dapat dibatalkan.
                </p>
                ...
                <label className="inline-flex px-8 py-4 bg-white border-2 border-slate-200 text-[#001a33] font-black uppercase text-xs tracking-widest rounded-2xl items-center gap-3 hover:border-rose-400 hover:text-rose-600 transition-all cursor-pointer shadow-sm active:scale-95">
                    ...
                    <input type="file" className="hidden" accept=".json" onChange={handleRestore} disabled={loading} />
                </label>
            </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm">
            <h4 className="font-bold text-[#001a33] flex items-center gap-3 mb-8">
                <ClockCounterClockwise weight="bold" className="text-xl text-slate-400" /> Riwayat Aktivitas Sistem
            </h4>
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody className="text-slate-600">
                        {activeHistory.map(log => (
                            <tr key={log.id} className="border-b border-slate-50 last:border-0 group">
                                <td className="py-6 px-4 font-bold text-[#001a33]">{log.type}</td>
            ...
            <div className="mt-10 p-6 bg-amber-50 rounded-3xl border border-amber-100 flex gap-6 items-center">
                <div className="p-4 bg-white rounded-2xl text-amber-500 shadow-sm"><ShieldCheck size={32} weight="duotone" /></div>
                <div>
                   <h5 className="font-black text-amber-900 text-sm mb-1 uppercase tracking-tight">Kebijakan Retensi Data</h5>
                   <p className="text-xs text-amber-700/70 leading-relaxed font-medium">Backup otomatis dilakukan setiap hari Minggu pukul 00:00 UTC ke cloud storage Amazon S3. Pastikan kunci API S3 terkonfigurasi pada tab integrasi.</p>
                </div>
            </div>
        </div>
