...
export default function IncomeReportPage() {
  ...
  const calculateIncome = (data: any[], campusId: string) => {
    let target = data;
    if (campusId !== "all") {
  ...
  if (loading) {
      return <div className="flex h-[80vh] items-center justify-center"><Loader2 className="animate-spin w-8 h-8 text-blue-900" /></div>;
  }
    ...
    <div className="space-y-6">
        <Card className="border-slate-200 shadow-sm p-4">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <h2 className="text-lg font-black uppercase text-blue-900">Report Revenue Global</h2>
                <div className="flex gap-2 w-full md:w-auto">
                    <Button variant="outline" className="text-xs font-bold uppercase text-slate-600 bg-slate-50 flex-1 md:flex-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="text-center border-b-4 border-b-blue-900 shadow-sm">
                <CardContent className="p-6">
                    <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Total Pemasukan</p>
                    <h3 className="text-3xl font-black text-blue-900 break-words">Rp {totalIncome.toLocaleString('id-ID')}</h3>
                </CardContent>
            </Card>
            <Card className="text-center border-b-4 border-b-blue-500 shadow-sm">
                <CardContent className="p-6">
                    <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Internal Conversion</p>
                    <h3 className="text-3xl font-black text-blue-600 break-words">Rp {internalIncome.toLocaleString('id-ID')}</h3>
                </CardContent>
            </Card>
            <Card className="text-center border-b-4 border-b-amber-500 shadow-sm">
                <CardContent className="p-6">
                    <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Lead Generation</p>
                    <h3 className="text-3xl font-black text-amber-600 break-words">Rp {leadsIncome.toLocaleString('id-ID')}</h3>
                </CardContent>
            </Card>
        </div>
        ...
        <Card className="border-slate-200 shadow-sm h-96 p-4">
            <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                    ...
                    <RechartsTooltip 
                        formatter={(val: number | undefined) => `Rp ${(val || 0).toLocaleString('id-ID')}`} 
