    ...
    <section id="kampus" className="py-20 bg-white border-t border-slate-100">
      <div className="max-w-6xl mx-auto px-6 text-center">
        <h2 className="text-2xl font-bold text-slate-900 mb-2">
          Mitra Kampus Terpercaya
        </h2>
        <p className="text-slate-500 mb-10">
          Bergabung dengan jaringan perguruan tinggi digital terbaik di
          Indonesia.
        </p>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 items-center justify-items-center">
            {campuses.map((campus: Campus) => (
              <div
                key={campus.id}
                className="flex flex-col items-center gap-3 group cursor-pointer hover:opacity-100 transition-opacity"
              >
                <div className="w-16 h-16 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 font-bold text-xl group-hover:bg-brand-50 group-hover:text-brand-600 transition-colors border border-transparent group-hover:border-brand-200">
                  {campus.logoPath ? (
                    <img
                      src={`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/storage/${campus.logoPath}`}
                      alt={campus.name}
                ...
                <span className="text-xs font-bold text-slate-600 max-w-30 leading-tight group-hover:text-brand-600 transition-colors">
                  {campus.name}
                </span>
              </div>
            ))}
